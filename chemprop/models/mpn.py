...
def tensor_filter(t,bool_vector,reverse):
    ...
    mask = bool_vector.view(-1, 1).expand_as(t)  
    filtered_tensor = t.clone()  
    if reverse:
...
class MPN(nn.Module):
    def __init__(self,
        ...
        self.frag_encoder = MPNEncoder(self.args, 194, 34)
    def forward(self, batch: Union[List[str], BatchMolGraph],
                features_batch: List[np.ndarray] = None) -> torch.FloatTensor:
        if not self.graph_input:  # if features only, batch won't even be used
            atom_batch = mol2graph(batch, self.args)
            fragment_batch = mol2graph(batch,self.args,brics = True)

        output = self.encoder.forward(atom_batch, features_batch)
        if self.args.multi_view ==0:
            return output
        frag_output = self.frag_encoder.forward(fragment_batch,features_batch)

        return torch.cat([output, frag_output], 1)
